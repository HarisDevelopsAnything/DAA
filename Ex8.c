#include <stdio.h>
#include <stdlib.h>
void printGraph(int **graph, int V) {
    printf("\nCurrent Residual Graph:\n");
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            printf("%d\t", graph[i][j]);
        }
        printf("\n");
    }
}
void printPath(int parent[], int t, int s) {
    if (t == s) {
        printf("%d", s);
        return;
    }
    
    printPath(parent, parent[t], s);
    printf(" -> %d", t);
}
int dfs(int **rGraph, int V, int s, int t, int parent[], int visited[]) {
    visited[s] = 1;
    if (s == t)
        return 1;
    for (int v = 0; v < V; v++) {
        if (visited[v] == 0 && rGraph[s][v] > 0) {
            parent[v] = s;
            if (dfs(rGraph, V, v, t, parent, visited))
                return 1;
        }
    }
    return 0;
}
int fordFulkerson(int **graph, int V, int s, int t) {
    int u, v;
    int **rGraph = (int**)malloc(V * sizeof(int*));
    for (u = 0; u < V; u++) {
        rGraph[u] = (int*)malloc(V * sizeof(int));
        for (v = 0; v < V; v++) {
            rGraph[u][v] = graph[u][v];
        }
    }
    int *parent = (int*)malloc(V * sizeof(int)); 
    int *visited = (int*)malloc(V * sizeof(int));
    int max_flow = 0;
    int iteration = 0; 
    printf("\n--- Starting Ford-Fulkerson Algorithm ---\n");
    printf("Initial res. graph is the same as the input graph\n");
    printGraph(rGraph, V);
    while (1) {
        iteration++;
        for (int i = 0; i < V; i++) {
            visited[i] = 0;
            parent[i] = -1;
        }
        if (!dfs(rGraph, V, s, t, parent, visited)) {
            printf("\nIteration %d: No more augmenting paths found.\n", iteration);
            break; // No more augmenting paths
        }   
        int path_flow = 99999;
        for (v = t; v != s; v = parent[v]) {
            u = parent[v];
            path_flow = (rGraph[u][v] < path_flow) ? rGraph[u][v] : path_flow;
        }
        printf("\n--- Iteration %d ---\n", iteration);
        printf("Augmenting path found: ");
        printPath(parent, t, s);
        printf("\n");
        printf("Bottleneck capacity of this path: %d\n", path_flow);
        for (v = t; v != s; v = parent[v]) {
            u = parent[v];
            rGraph[u][v] -= path_flow;
            rGraph[v][u] += path_flow;
        }
        max_flow += path_flow;
        printf("Flow sent along this path: %d\n", path_flow);
        printf("Total flow so far: %d\n", max_flow);   
        printGraph(rGraph, V);
    }
    return max_flow;
}
int main() {
    int V, s, t;
    printf("Enter the no. of vertices: ");
    scanf("%d", &V);
    int **graph = (int**)malloc(V * sizeof(int*));
    for (int i = 0; i < V; i++) {
        graph[i] = (int*)malloc(V * sizeof(int));
    }
    printf("Enter the source vertex (0 to %d): ", V-1);
    scanf("%d", &s);
    printf("Enter the sink vertex (0 to %d, other than %d): ", V-1, s);
    scanf("%d", &t);
    if(t==s){
        printf("Source and sink can't be same.\n");
        exit(0);
    }
    printf("Enter the capacity of each edge (enter 0 for no edge):\n");
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            if(i != j) {
                printf("Capacity from vertex %d to vertex %d: ", i, j);
                scanf("%d", &graph[i][j]);
            }
            else
                graph[i][j] = 0;
        }
    }
    printf("\nInput Graph (Capacity Matrix):\n");
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            printf("%d\t", graph[i][j]);
        }
        printf("\n");
    }
    int max_flow = fordFulkerson(graph, V, s, t);
    printf("\nThe maximum possible flow is %d\n", max_flow);
    return 0;
}
